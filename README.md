...
| `python3 okf.py links <bundle> [--dangling]` | Three groups: broken body links (tolerated, §6.1), path-valued fields pointing nowhere inside the bundle (`computation`, `executor.resource`, `attester.resource`, explicit-path `sources[].resource`), and path-valued fields that escape the bundle root with no file there. **Exit 1 only for the last group** — a missing external source is provenance breakage, not a placeholder. `--dangling` groups unresolved internal targets by directory with reference counts and referrers, for converging slugs across several writers. | OKF §6.1 tolerates broken links as markers for not-yet-written knowledge — review, don't auto-fix. |
...
| `python3 okf.py tags <bundle> [--registry <file>]` | Inventories every tag in use — single-use tags are listed as inventory, not defects; with a registry (given or the bundle's own), exit 1 on unregistered tags. | Keeps labels useful as routing keys instead of decaying into one-tag-per-concept. |
| `python3 viz.py <bundle> [--out <path>] [--name <title>] [--exclude-type <T>]... [--exclude-edge <link\|source>]...` | Renders the bundle as an interactive graph (excluded types drop with every edge touching them, before serialization, so the inline data shrinks) — concepts as nodes, links as edges, source derivations as dashed edges; status / trust-tier / stale chips and a provenance block per concept. | Single self-contained HTML file, no external assets — open it in any browser. |

Every command answers `--help` with its flags: `python3 okf.py links --help`.
